...
URL 정규화에 사용하는 사용자 토큰, 비공개 모듈 본문, 비공개 저장소·API 설정은 `run_context.json`의 일부가 아니다. 실행마다 사용자 프로필의 비공개 환경 파일을 읽고 GitHub API 응답을 메모리에서만 처리하며, 이 값들을 컨텍스트·manifest·결과·로그에 복사하거나 캐시하지 않는다.

## 생성 순서

1. 최종보고서에서 작업일을 추출한다.
2. `작업내역:` XLSX가 명시되면 그 파일에서, 생략되면 Google Sheets에서 작업일 전날 보도분과 좁은 하루 전 작업일 재반영 후보를 가져온다. 명시 파일의 실패는 시트 조회로 대체하지 않는다. XLSX 입력은 두 Python 단계 모두 `--source-xlsx`로 전달해 원본을 입력 지문·manifest에 포함한다. 정기 JSON도 원본에서 다시 읽은 필터 결과 전체와 대조하며, 컨텍스트 작성 후 원본 변경을 거부한다. `source_history`에는 선택 모드(`local_xlsx` 또는 `google_sheets`), 원본 경로와 해시가 기록된다.
...
- 정기 유입은 `정기/오후/총괄/<당일 오후 취합 작업자>`, 일본언론동향은 작업조 `일본문화원`, 순방 파일은 작업조 `순방`을 사용한다. 일본·순방의 담당과 작업자는 실제 당일 편집 작업본으로 판정한다. 일본동향의 작업일 당일 보도분이 오전 보조본에서 실제 본문과 함께 확인되면 `오전/보조`, 전일 보도분은 `보조`로 기록한다.
- 정기·당일 외신동향에서 본문이 실제로 작성된 비유사 개별 초안이 자동 기준 이상으로 중복되면 실제 동향 초안을 우선한다. 최종보고서에 정규화 URL이 있으면 같은 URL 군이어야 하며, 본문 없는 유사보도 제목줄은 이 우선 규칙의 개별 초안이 아니다. 정기 전일 보도분이 오후와 오전 개별·보조 초안에는 없고 오전 총괄본에서 처음 재반영된 좁은 예외만 `정기/오전/총괄/<오전 총괄 작업자>`를 허용한다.
...
- 본문이 실제로 작성된 비유사 개별 국내·글로벌 동향 초안이 정기와 자동 기준 이상으로 중복된 경우에는 동향 초안을 우선한다. 최종보고서에 정규화 URL이 있으면 그 URL까지 같은 군이어야 한다. 제목수정으로 직접 점수가 낮으면 같은 매체·날짜의 초안이 하나이고 의미 있는 제목 관계가 남으며 후속 취합본에서 최종 제목으로 강하게 이어진 경우에만 동일 중복 계보로 인정한다. 이 예외가 아니면 현재 단계에서 기준 점수 이상의 후보가 확인됐다는 이유만으로 뒤 단계의 취합·총괄본이 더 높은 점수나 `priority`를 가져도 선택을 바꾸지 않는다.
