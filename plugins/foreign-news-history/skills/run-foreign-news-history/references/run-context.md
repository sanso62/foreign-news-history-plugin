...
- 파일 단계별 기본 역할은 고정된 사람 매핑이 아니라 업무 프롬프트의 열 판정 규칙이다: 오후 국내 초안 `1조/국내`, 오후 글로벌 작업본 `1조/글로벌`, 오후 취합본 `1조/오후/총괄`, 오전 보조·초안 `2조/보조`, 오전 n차·최종 총괄본 `2조/오전/총괄`.
...
- 현재 단계에서 기준 점수 이상의 후보가 확인되면 뒤 단계의 후보가 더 높은 점수나 `priority`를 갖더라도 선택을 바꾸지 않는다.
