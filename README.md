...
현재 배포 버전은 `v0.1.7`입니다. `작업내역`은 표시 날짜 문자열 검색 대신 메타데이터 경계 안의 `A:K` 전체를 유한 범위로 읽고 로컬에서 대상일을 파싱하며, 감사정보 없는 부분 조회 입력은 거부합니다. `/` 묶음 대표 제목은 첫 매체 기사에 적용하고 일본언론동향은 전체 기사 묶음을 가장 완전하게 취급한 당일 작업본의 담당자를 사용합니다. 제목이 크게 바뀐 같은 매체·인접 날짜의 일본동향 후보는 원문 대조 전까지 검수 대상으로 유지합니다. 최종보고서 기사의 유입 경로는 다음 시간 순서로 비교합니다.
