...
현재 배포 버전은 `v0.1.4`입니다. `근무` 탭의 `동향 스케줄` 위치가 바뀌어도 매 실행 메타데이터와 현재 그리드에서 표 제목·헤더를 다시 찾아 실제 셀 근거를 보존합니다. 파일 단계와 당일 근무표를 대조해 작업조·담당·작업자를 판정하며, 최종보고서 기사의 유입 경로는 다음 시간 순서로 비교합니다.
