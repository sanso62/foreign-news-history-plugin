...
현재 배포 버전은 `v0.1.5`입니다. 플러그인에 HWP 판독 모듈을 포함하고, 작업본·일본동향 파일이 읽히지 않거나 기사 수가 0건이면 잘못된 역할을 만들기 전에 실행을 중단합니다. 작업조·초벌 담당·초벌 작업자는 실제 파일 단계와 당일 근무표 근거로 검증하며, 확인 필요 행이 남으면 Google Sheets 반영을 차단합니다. 최종보고서 기사의 유입 경로는 다음 시간 순서로 비교합니다.
